...
def _select_anime_title(message: Message) -> None:
    ...
    bot.register_next_step_handler(message, 
                                    _show_anime_osts, search_results) 
...
def _show_anime_osts(message: Message, anime_info: list) -> None:
    ...
    bot.send_message(message.chat.id, 
                        f"Anime Search results:{anime_info[index][0]}")

    songs = anime_handler.osts(anime_info[index][1])
...
def _ask_anime_title(message: Message, func) -> None:
    bot.send_message(message.chat.id,
